...
try:
    sp_api = spotipy.Spotify(auth_manager=SpotifyClientCredentials(
        client_id=music_config.SPOTIFY_ID, client_secret=music_config.SPOTIFY_SECRET))
    api = True
except:
    api = False
    ...
    if api == True:
        ...
        if "open.spotify.com/album" in url:
            try:
                ...
                if music_config.SPOTIFY_ID != "" or music_config.SPOTIFY_SECRET != "":
...
def identify_url(url):
    ...
    if url.lower().endswith(music_config.SUPPORTED_EXTENSIONS):
        return Sites.Custom
