...
class AudioController(object):
    ...
    def track_history(self):
        history_string = music_config.INFO_HISTORY_TITLE
    ...
    async def play_song(self, song):
        ...
        if song.info.title == None:
            ...
            try:
                downloader = yt_dlp.YoutubeDL(
                    {'format': 'bestaudio', 'title': True, "cookiefile": music_config.COOKIE_PATH})
                r = downloader.extract_info(
                    song.info.webpage_url, download=False)
            except:
                asyncio.wait(1)
                downloader = yt_dlp.YoutubeDL(
                    {'title': True, "cookiefile": music_config.COOKIE_PATH})
                r = downloader.extract_info(
        ...
        for song in list(self.playlist.playque)[:music_config.MAX_SONG_PRELOAD]:
    ...
    async def process_playlist(self, playlist_type, url):
        ...
        if playlist_type == linkutils.Playlist_Types.YouTube_Playlist:
            ...
            options = {
                ...
                'extract_flat': True,
                "cookiefile": music_config.COOKIE_PATH
            }
    ...
    async def preload(self, song):
        ...
        executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=music_config.MAX_SONG_PRELOAD)
    ...
    async def uconnect(self, ctx):
        ...
        if self.guild.voice_client == None:
            ...
            await ctx.send(music_config.ALREADY_CONNECTED_MESSAGE)

    async def udisconnect(self, ctx):
        await self.stop_player()
        await self.guild.voice_client.disconnect(force=True)
        await ctx.send("Left voice and cleared music.")
