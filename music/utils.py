...
async def connect_to_channel(guild, dest_channel_name, ctx, switch=False, default=True):
        ...
        if str(channel.name).strip() == str(dest_channel_name).strip():
            if switch:
                try:
                    ...
                    await ctx.send(music_config.NOT_CONNECTED_MESSAGE)
    ...
    if default:
        try:
            ...
            await ctx.send(music_config.DEFAULT_CHANNEL_JOIN_FAILED)
    else:
        await ctx.send(music_config.CHANNEL_NOT_FOUND_MESSAGE + str(dest_channel_name))
...
async def play_check(ctx):
    ...
    if cm_channel != None:
        if cm_channel != ctx.message.channel.id:
            await ctx.send(music_config.WRONG_CHANNEL_MESSAGE)
            return False
    ...
    if vc_rule == True:
        ...
        if author_voice == None:
            await ctx.send(music_config.USER_NOT_IN_VC_MESSAGE)
            return False
...
class Timer:
    ...
    async def _job(self):
        await asyncio.sleep(music_config.VC_TIMEOUT)
