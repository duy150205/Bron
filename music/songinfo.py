...
class Song():
    ...
    class Sinfo:
        ...
        def format_output(self, playtype):
            ...
            embed = discord.Embed(title=playtype, description="[{}]({})".format(self.title, self.webpage_url), color=music_config.EMBED_COLOR)

            if self.thumbnail is not None:
                embed.set_thumbnail(url=self.thumbnail)

            embed.add_field(name=music_config.SONGINFO_UPLOADER,
                            value=self.uploader, inline=False)

            if self.duration is not None:
                embed.add_field(name=music_config.SONGINFO_DURATION,
                                value="{}".format(str(datetime.timedelta(seconds=self.duration))), inline=False)
            else:
                embed.add_field(name=music_config.SONGINFO_DURATION,
                                value=music_config.SONGINFO_UNKNOWN_DURATION , inline=False)
