...
class Settings():
    ...
    async def format(self):
        embed = discord.Embed(
            title="Settings", description=self.guild.name, color=music_config.EMBED_COLOR)

        embed.set_thumbnail(url=self.guild.icon_url)
        embed.set_footer(
            text="Usage: {}set setting_name value".format(music_config.BOT_PREFIX))
    ...
    async def default_nickname(self, setting, value, ctx):
        ...
        if len(value) > 32:
            await ctx.send("`Error: Nickname exceeds character limit`\nUsage: {}set {} nickname\nOther options: unset".format(music_config.BOT_PREFIX, setting))
            return False
        else:
    ...
    async def user_must_be_in_vc(self, setting, value, ctx):
        if value.lower() == "true":
            ...
            await ctx.send("`Error: Value must be True/False`\nUsage: {}set {} True/False".format(music_config.BOT_PREFIX, setting))
            return False
    ...
    async def default_volume(self, setting, value, ctx):
        try:
            ...
            await ctx.send("`Error: Value must be a number`\nUsage: {}set {} 0-100".format(music_config.BOT_PREFIX, setting))
            return False
    ...
    async def vc_timeout(self, setting, value, ctx):
        ...
        if music_config.ALLOW_VC_TIMEOUT_EDIT == False:
            await ctx.send("`Error: This value cannot be modified".format(music_config.BOT_PREFIX, setting))
